...
Depending on the Game Boy model or flash cartridge you use, it is possible tu run the device from the +5V of the serial port only. For this, I recommend using a version of the TinyGB Printer **without the display** and if possible **non SD flash cartridge** (so no EZ-FLASH Junior for example). It should put you in a situation where enough juice is available from that poor internal Game Boy power converter.
