...
You can also share your images with the friendly community of Game Boy Camera enthusiasts on the [Game Boy Camera Club Telegram](https://t.me/gameboycamera). I can also help you solving issues on that Telegram. Just to precise: I do not sell the device, buying parts, making and sending them at a decent price would not cover my working hours.
...
- [A 240x240 1.3 inches TFT Display](https://aliexpress.com/w/wholesale-240x240-pixels-7-pins.html). It must be that exact one, 7 pins, without CS, 240x240 pixels, GMT130-V1.0 (now often IPS 240x240 apparently). If you decide to use another 240x240 pixels model, CS pin is exposed and driven but it's up to you to reconfigure the TFT library if necessary.
...
- [A 5V DC-DC step-up converter](https://www.aliexpress.com/item/32809095271.html), 0.9-5V (input) to 5V (output). I know, it does not look impressive but it does perfectly the job of powering small devices. If you want the blue fullset, they sometimes appear with blus PCB.
...
- The device uses a quite baroque 7 pins 240x240 TFT display without CS pin. This is just because I had this one in stock as leftover from another project during dev. It is also very tiny and cute. This display is hopefully quite common on Aliexpress but on the other hand requires some annoying tuning of the TFT library (which in return prevents very efficiently people to alter the design ^_^). The project PCB and code can be easily modified to handle more classical TFT display.
