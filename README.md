...
- The inspiration came from the feedback after developping the NeoGB Printer on ESP32: the default PCB is bulky and difficult to assemble (the random pin placement on the ESP dev board does not help), the device consumes a ton of current due to wifi support (which is not that reliable), the attempts to port it to a compact LilyGO board were more or less a fail because **ALL** these boards have majors issues with voltage charge/discharge circuit (plus they are horribly expensive), the dev on ESP32 was a real nightmare (you have to surf between major hardware flaws and very questionable "documentation"). The Tiny GB Printer may probably be ported easily on the ESP32 mini, but it won't be by me.
