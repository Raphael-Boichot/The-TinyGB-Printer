...
- All genuine games uses packets of 640 bytes as payload to the printer (natural size to print with one pass of the printer head which is 16 pixels high) but in theory it could be different. An homebrew uses packets of 320 bytes (blarble1290). My own tests show that a real printer (Hosiden series) refuses to print with this game. I guess that it was tested on emulator or on a Seiko series only. The TinyGB printer was modified to accept this particular weird protocol, and it was a pain.
