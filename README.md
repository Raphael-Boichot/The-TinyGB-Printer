...
- The [DC-DC converter](/Images/DC_DC_characteristics.png) allows in theory using from 1 to 4 NiMH batteries. The bottleneck is how current much draws the system in the worse moment: when the PNG file is written to the SD card. It's about 200 mA peak. If voltage drops during this phase, the file is just discarded. 2 cells is a good reliability / bulkiness compromise from my own experience but 3 to 4 would allow going farther in the discharge curve. One single cell is just not an option.
