...
- [3 x 22 µF caps with a 1206 footprint](https://fr.aliexpress.com/item/1005006022131059.html) (10 to 50 µF is OK if you have spares).
...
- The inspiration came from the feedback after co-developping the NeoGB Printer on ESP32: the default PCB is bulky and difficult to assemble (the random pin placement on the ESP dev board does not help), the device consumes a ton of current due to wifi support (which is not that reliable), the attempts to port it to a compact LilyGO board were more or less a fail because **ALL** the boards from this series have majors issues with their charge/discharge circuit (plus they are horribly expensive for what they are: piece of crap), the dev on ESP32 was a real nightmare (you have to surf between major hardware flaws and very questionable "documentation", things that must be working just don't work because, ram memory management is awful, etc.). The Tiny GB Printer may probably be ported easily on the ESP32 zero, but it won't be by me, never. Good luck if you try.
...
- [Toxa](https://github.com/untoxa) for helping me understand the PNG format subtilities.
