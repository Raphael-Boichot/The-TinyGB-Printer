...
This emulator does not require any converter, it's like a [BitBoy](https://gameboyphoto.bigcartel.com/product/bitboy) but open access (and much cheaper). This allows people working on a diversity of OS to get their images out of their precious saves without the hassle of installing / configuring / running a decoder. Here everything is simple: build the device, drop the compiled binary to it, print, and publish immediately your images from the SD card. Only skill required is very basic soldering skill.
