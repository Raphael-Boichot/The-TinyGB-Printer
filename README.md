...
- For obscure reasons probably linked to the level shifter used (and its LOW/HIGH level thresholds), to date, the Analogue Pocket refuses to trigger communication with the TinyGB Printer. I however do not (and probably will never) have the real device to investigate the issue. I guess that not using the level shifter and shorting the LVx<->HVx (x=1 to 4) lines must solve the problem if this is what I think... It is anyway possible to directly extract Game Boy Camera images from the Analogue Pocket savestates. Bummer for the other compatible games.
