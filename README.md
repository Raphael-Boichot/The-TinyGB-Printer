...
- The break command (pressing B while printing to abort) is only partially supported and may lead to glitches in the next image to print (or no image at all). Just reboot after an abort and everything will be fine. This issue may be fixed in the future.
