...
void loop1()  //core 1 loop written by Raphaël BOICHOT, november 2024
{
  ...
  if (PRINT_flag == 1) {
    ...
    //delay(500);                                                 //this is to delay the jamming due to SD access within a DATA packet to avoid possible protocol crash
    PRINT_flag = 0;
    //preparing palette;
    ...
    if ((inner_lower_margin > 0) & (TEAR_mode == 0)) {  //the printer asks to feed paper, end of file, except in TEAR mode
      ...
      lines_in_bmp_file = 0;
#ifdef PNG_OUTPUT
      png_upscaler(storage_file_name, png_storage_file_name, 4,BMP_palette);
      SD.remove(storage_file_name);
#endif
    }
