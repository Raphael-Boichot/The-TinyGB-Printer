...
void png_upscaler(char BMP_input[], char PNG_output[], unsigned int upscaling_factor, unsigned char PNG_palette[]) {
  ...
  if (skip == 0) {
    ...
    for (unsigned int y = 0; y < BMP_h; y++) {  //treats a line
      ...
      //   if (pixel_gray_level == PNG_palette[3]) color_index = 0x03;  //converts BMP gray level to color PNG color index
      //   if (pixel_gray_level == PNG_palette[2]) color_index = 0x02;  //converts BMP gray level to color PNG color index
      //   if (pixel_gray_level == PNG_palette[1]) color_index = 0x01;  //converts BMP gray level to color PNG color index
      //   if (pixel_gray_level == PNG_palette[0]) color_index = 0x00;  //converts BMP gray level to color PNG color index
